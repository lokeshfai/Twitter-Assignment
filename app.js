...
const tweetAccessVerification = async (request, response, next) => {
  ...
  const getTweetQuery = ` SELECT
    * FROM tweet INNER JOIN follower
    ON tweet.user_id = '${tweetId}' AND follower_user_id = '${userId}';`;
...
app.get("/user/following/", authentication, async (request, response) => {
  ...
  const getFollowingUserQuery = `SELECT name
    FROM follower INNER JOIN user ON user.user_id = following_user_id
...
app.get("/user/followers/", authentication, async (request, response) => {
  const { username, userId } = request;
  const getFollowersUserQuery = `SELECT DISTINCT name
